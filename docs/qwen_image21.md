...
The September 20 SwarmUI compatibility assessment is superseded by the September 22 [QwenImage21 premium extension](https://github.com/FurkanGozukara/SwarmUI_Premium_Extensions/tree/main/QwenImage21). SwarmUI `163503eb` registers Qwen 2.1 models but its generic graph still differs from this unified workflow. The extension reuses these Prepare/Finish nodes and native Qwen inference without changing either application's core. `SEQwenImage21SwarmInputs` decodes ordered Swarm attachments and init as RGB/RGBA and decodes its white-to-edit mask as grayscale; it does not use the gallery file resolver. Ten regression tests cover both gallery and tensor inputs. The extension README documents installer delivery and live Swarm validation.
...
## HQ two-pass preset, 2026-09-22

`Presets/Qwen Image 2.1 - Unified Generate Edit Inpaint RGBA HQ - 260922.json` keeps the unified generate/edit/inpaint/RGBA first pass at the 1024 base/reference size, then runs native `ImageScaleBy` (Lanczos, editable `scale_by` default 2.0), `VAEEncode`, and a second 25-step Euler/simple/CFG-1 `KSampler` with editable denoise default 0.30. The existing finish/composite node runs before the upscale so masked mode restores the first-pass unmasked area at its working resolution; the requested final img2img stage then refines the entire enlarged result and may make small changes outside the original mask.

The default text-only path was executed through the live ComfyUI 0.37.0 API on physical GPU 0, pinned by RTX 5090 UUID `GPU-21350b86-3099-1af0-58ef-f6432e60d9d0`. It completed in 49.20 seconds and saved a 2048x2048 four-channel sRGBA PNG at `output/Qwen_Image_2.1/validation/HQ_preset_1024_2x_030_00001_.png` (SHA-256 `C30A7BAEA070808453C67C938E1A16EFCFA4914BA4CDC62B8358AE81E5046F09`). The embedded API prompt records `scale_by: 2.0` and second-pass `denoise: 0.3`; the result was visually checked. No custom-node code change was required.
